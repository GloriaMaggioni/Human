...
export class UsersService  {
   ...
   totalUser$ = new BehaviorSubject<number>(0)

   // metodo per aggiunger un nuovo user (call api)
   addUser(body: Partial<User>){
    return this.http.post<User>(this.apiUrl, body, {headers: this.headers})
   }


   // metodo per prendere i dati degli user
   getUser(pageNumber : number = 1){
    this.http.get<User[]>(`https://gorest.co.in/public/v2/users?page=${pageNumber}&per_page=30`, {headers: this.headers, observe: 'response'}).subscribe({
      next: (response : any) =>{
